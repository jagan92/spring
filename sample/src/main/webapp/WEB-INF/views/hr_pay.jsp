...
<!-- 										placeholder="Consultant ID" /> -->
										<form:select id="id_select_consultant" path="consultantId" class="form-control"  onchange="loadCosultant(this)">
											...
											<c:forEach items="${consultantVOs}" var="consultantVO">
												<option value="${consultantVO.id}" <c:if test="${consultantVO.id == hrPayVO.consultantId}"> selected</c:if> > TWE${consultantVO.id} </option>
											</c:forEach>
										</form:select>
								</div>
							</div>
...
<form:input type="text" class="form-control" id="departing_date"     path = "trans_date"
              ...
              <%-- <c:if test="${HrPayVO.rate_per = Hour }">  --%>
              	<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" class="form-control" path="last_name"  id="id_num_hours"
											placeholder="hours" />
									</div>
								</div>
            ...
            <%--  </c:if>  --%>
              	<div class="col-md-3">
									<div class="form-group">
										...
										<input type="text" class="form-control" path="last_name" id="id_num_days"
											placeholder="days" />
									</div>
								</div>
              	...
              	<div class="col-md-3" >
								 ...
								 <div class="col-md-4">
									<div class="form-group">
										<label for="exampleInputEmail1"style="font-size: 21px;">TDS Reduction</label>
										<input type="text" class="form-control" id="id_tds_deduction"
               ...
               <div class="col-md-5">
              <div class="col-md-2" >
									<button type="button" class="btn btn-success" style="margin-top: 11%; " data-conId="${hrPayVO.consultantId}" onclick="loadMail(this)"  >Send Mail</button>
			 
								</div>
			<div class="col-md-2" >
									<button type="submit" id="id_pain_btn" class="btn btn-success" style="margin-top: 11%;">Paid</button>
								</div>	
								<div class="col-md-2" >
									<button type="button" class="btn btn-success" style="margin-top: 12%;
	...
	function funcCalculateDayPay(){
		...
		$("#id_tds_deduction").val(tdsAmt);
		$("#id_take_home").val(takeHome);
		
		var lsDate = $("#departing_date").val();
	...
	function funcCalculateHourPay(){
	   ...
	   function loadMail(e){
		   var conId = $(e).attr("data-conId");
			location.href="hr_pay_email.htm?consultantId="+conId;
		}
		  ...
		  $(function(){
			  setTimeout(function(){
				  var lsRatePer = $("#id_rate_per").val();
				 	$("#id_num_days").prop('disabled', false);
					$("#btn_num_days").prop('disabled', false);
					 $("#id_num_hours").prop('disabled', false);
						$("#btn_num_hours").prop('disabled', false);
				 if(lsRatePer == "Day"){
					 $("#id_num_hours").prop('disabled', true);
					$("#btn_num_hours").prop('disabled', true);
				 } else if(lsRatePer == "Hour"){
					 $("#id_num_days").prop('disabled', true);
